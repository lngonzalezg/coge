                ...
                */
                ...
                renderFeature: function( feature, uniqueId, block, scale, labelScale, descriptionScale, containerStart, containerEnd ) {
                    ...
                    if ((name || description) && this.showLabels && isType(geneTypes)) {// && scale >= labelScale ) {
                        ...
                        var exists = false;
                        // block.domNode.parentElement is null when this async feature
                        // fill completes after the block was detached (fast scrolling)
                        // -- the render is moot then, but the label-dedup scan below
                        // crashed with "Cannot read properties of null" on every such
                        // block. Treat a detached block as having no existing labels.
                        var blockParent = block.domNode.parentElement;
                        var labels = blockParent ? blockParent.querySelectorAll('.feature-label') : [];
                        for (var i=0; i<labels.length; i++)
